...
export function PonsEyeLab({ tokens }: { tokens: LabToken[] }) {
  const [labTokens, setLabTokens] = useState(tokens);
  const [surveillanceGate, setSurveillanceGate] = useState<SurveillanceGateSettings>({ ...defaultSurveillanceGate });
  ...
  useEffect(() => {
    try {
      ...
      const storedGate = JSON.parse(window.localStorage.getItem(currentSurveillanceGateKey) ?? "null") as Partial<SurveillanceGateSettings> | null;
      ...
      if (storedGate) {
        const restoredGate = {
          minAgeSeconds: Number(storedGate.minAgeSeconds ?? defaultSurveillanceGate.minAgeSeconds),
          maxAgeSeconds: Number(storedGate.maxAgeSeconds ?? defaultSurveillanceGate.maxAgeSeconds),
          minMarketCapUsd: Number(storedGate.minMarketCapUsd ?? defaultSurveillanceGate.minMarketCapUsd),
          minTrades: Number(storedGate.minTrades ?? defaultSurveillanceGate.minTrades),
          minUniqueTraders: Number(storedGate.minUniqueTraders ?? defaultSurveillanceGate.minUniqueTraders),
          minBuyPressurePct: Number(storedGate.minBuyPressurePct ?? defaultSurveillanceGate.minBuyPressurePct),
          requireNoCreatorSales: storedGate.requireNoCreatorSales !== false,
        };
        setSurveillanceGate(restoredGate);
      }
    } catch {
  ...
  useEffect(() => {
    if (!storageReady) return;
    window.localStorage.setItem(currentSurveillanceGateKey, JSON.stringify(surveillanceGate));
  }, [surveillanceGate, storageReady]);

  const analysis = useMemo(() => {
    const scored = labTokens.map((token) => scoreToken(token, settings));
  ...
  }, [labTokens, settings, exitSettings]);
  ...
  function updateSurveillanceGate<Key extends keyof SurveillanceGateSettings>(key: Key, value: SurveillanceGateSettings[Key]) {
    setSurveillanceGate((current) => {
      const next = { ...current, [key]: value };
      if (key === "minAgeSeconds" && next.maxAgeSeconds < next.minAgeSeconds) next.maxAgeSeconds = next.minAgeSeconds;
      if (key === "maxAgeSeconds" && next.minAgeSeconds > next.maxAgeSeconds) next.minAgeSeconds = next.maxAgeSeconds;
      return next;
    });
    setFullFunnelError("");
  }

  async function runFullFunnelTest() {
    setIsRunningFullFunnel(true);
    setFullFunnelError("");
    try {
      const response = await fetch("/api/lab/full-funnel", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(surveillanceGate),
      });
      const result = await response.json() as { tokens?: LabToken[]; settings?: SurveillanceGateSettings; error?: string };
      if (!response.ok || !Array.isArray(result.tokens)) throw new Error(result.error ?? "The replay did not return a dataset.");
      setLabTokens(result.tokens);
      setAppliedSurveillanceGate(result.settings ?? surveillanceGate);
    } catch (error) {
      setFullFunnelError(error instanceof Error ? error.message : "The full funnel replay could not be completed.");
    } finally {
      setIsRunningFullFunnel(false);
    }
  }

  const hitRate = analysis.selected.length ? analysis.hits.length * 100 / analysis.selected.length : 0;
  const surveillanceGateChanged = appliedSurveillanceGate == null || JSON.stringify(surveillanceGate) !== JSON.stringify(appliedSurveillanceGate);
    ...
    <>
      <section className="labModelDock">
        ...
        {controlTab === "surveilling" && (
          <section className="labStagePanel">
            ...
            <p>Change the gate, then run the full funnel. The Lab finds the first moment each Sighted launch would have qualified and replays everything after it.</p>
            <div className="labRuleHeading labFastHeading"><span>All rules required</span><span>Pass mark</span></div>
            <div className="labRules labFastRules labSurveillanceRules">
              <section className="labRule">
                <div className="labRuleTitle"><strong>Earliest launch age</strong><span>Do not promote before this age</span><small>Minimum required</small></div>
                <label className="labRuleValue"><span><input aria-label="Earliest launch age in seconds" type="number" min="60" max="600" step="30" value={surveillanceGate.minAgeSeconds} onChange={(event) => updateSurveillanceGate("minAgeSeconds", Math.max(60, Math.min(600, Number(event.target.value) || 60)))} />s</span><input aria-label="Earliest launch age slider" type="range" min="60" max="600" step="30" value={surveillanceGate.minAgeSeconds} onChange={(event) => updateSurveillanceGate("minAgeSeconds", Number(event.target.value))} /></label>
              </section>
              <section className="labRule">
                <div className="labRuleTitle"><strong>Latest launch age</strong><span>Qualification window closes after this age</span><small>Maximum allowed</small></div>
                <label className="labRuleValue"><span><input aria-label="Latest launch age in minutes" type="number" min="1" max="60" step="1" value={surveillanceGate.maxAgeSeconds / 60} onChange={(event) => updateSurveillanceGate("maxAgeSeconds", Math.max(60, Math.min(3600, (Number(event.target.value) || 1) * 60)))} />m</span><input aria-label="Latest launch age slider" type="range" min="60" max="3600" step="60" value={surveillanceGate.maxAgeSeconds} onChange={(event) => updateSurveillanceGate("maxAgeSeconds", Number(event.target.value))} /></label>
              </section>
              <section className="labRule">
                <div className="labRuleTitle"><strong>Market cap</strong><span>USD market cap at the qualifying moment</span><small>Minimum required</small></div>
                <label className="labRuleValue"><span>$<input aria-label="Surveillance market cap" type="number" min="1000" max="100000" step="1000" value={surveillanceGate.minMarketCapUsd} onChange={(event) => updateSurveillanceGate("minMarketCapUsd", Math.max(1000, Math.min(100000, Number(event.target.value) || 1000)))} /></span><input aria-label="Surveillance market cap slider" type="range" min="1000" max="100000" step="1000" value={surveillanceGate.minMarketCapUsd} onChange={(event) => updateSurveillanceGate("minMarketCapUsd", Number(event.target.value))} /></label>
              </section>
              <section className="labRule">
                <div className="labRuleTitle"><strong>Trades</strong><span>All buys and sells recorded by qualification</span><small>Minimum required</small></div>
                <label className="labRuleValue"><span><input aria-label="Surveillance trades" type="number" min="1" max="100" step="1" value={surveillanceGate.minTrades} onChange={(event) => updateSurveillanceGate("minTrades", Math.max(1, Math.min(100, Number(event.target.value) || 1)))} /></span><input aria-label="Surveillance trades slider" type="range" min="1" max="100" step="1" value={surveillanceGate.minTrades} onChange={(event) => updateSurveillanceGate("minTrades", Number(event.target.value))} /></label>
              </section>
              <section className="labRule">
                <div className="labRuleTitle"><strong>Unique traders</strong><span>Different wallets seen by qualification</span><small>Minimum required</small></div>
                <label className="labRuleValue"><span><input aria-label="Surveillance unique traders" type="number" min="1" max="50" step="1" value={surveillanceGate.minUniqueTraders} onChange={(event) => updateSurveillanceGate("minUniqueTraders", Math.max(1, Math.min(50, Number(event.target.value) || 1)))} /></span><input aria-label="Surveillance unique traders slider" type="range" min="1" max="50" step="1" value={surveillanceGate.minUniqueTraders} onChange={(event) => updateSurveillanceGate("minUniqueTraders", Number(event.target.value))} /></label>
              </section>
              <section className="labRule">
                <div className="labRuleTitle"><strong>Buy pressure</strong><span>Buy trades as a percentage of all trades</span><small>Minimum required</small></div>
                <label className="labRuleValue"><span><input aria-label="Surveillance buy pressure" type="number" min="0" max="100" step="1" value={surveillanceGate.minBuyPressurePct} onChange={(event) => updateSurveillanceGate("minBuyPressurePct", Math.max(0, Math.min(100, Number(event.target.value) || 0)))} />%</span><input aria-label="Surveillance buy pressure slider" type="range" min="0" max="100" step="1" value={surveillanceGate.minBuyPressurePct} onChange={(event) => updateSurveillanceGate("minBuyPressurePct", Number(event.target.value))} /></label>
              </section>
            </div>
            <label className="labSurveillanceCreatorGate"><span><b>No creator sales</b><small>Require zero creator sells before qualification</small></span><input type="checkbox" checked={surveillanceGate.requireNoCreatorSales} onChange={(event) => updateSurveillanceGate("requireNoCreatorSales", event.target.checked)} /></label>
            <div className="labFullFunnelAction">
              <span><strong>{labTokens.length.toLocaleString()} launches reached Surveilling</strong><small>{surveillanceGateChanged ? "Rules changed. Run the test to update every later result." : "Current results use these Sighted → Surveilling rules."}</small></span>
              <button type="button" disabled={isRunningFullFunnel || !surveillanceGateChanged} onClick={runFullFunnelTest}>{isRunningFullFunnel ? "Replaying full funnel…" : surveillanceGateChanged ? "Run full funnel test" : "Full funnel up to date"}</button>
            </div>
            {fullFunnelError && <p className="labFullFunnelError">{fullFunnelError}</p>}
            <small className="labStageNote">Fresh evidence is automatic in the replay because every checkpoint is created by an actual trade. No future data is used to choose the qualifying moment.</small>
          </section>
        )}
      ...
      <section className="labOutput">
        <div className="labSummaryGrid">
          <article className="primary"><small>Would reach Acquired</small><strong>{analysis.selected.length}</strong><span>from {labTokens.length} Surveillance tokens</span></article>
        ...
        <section className="labResults">
          <header>
            <div className="labResultTabs">
              <button type="button" className={resultView === "all" ? "active" : ""} onClick={() => setResultView("all")}>View all <span>{labTokens.length}</span></button>
        ...
        <footer className="labMethodNote">
          ...
          <span>The Lab starts with every recorded launch and uses the first checkpoint that passes the Sighted → Surveilling gate. Later rules use only information available at that signal. Curve and canonical migrated pool prices are joined in USD. Holder rules are ignored when no holder snapshot existed at signal time.</span>
        </footer>
      </section>
    </>
  );
