...
async function main() {
  ...
  void getAccessToken()
    .then(async (auth) => {
      const repairFeed = "complete_market_history_v4";
    ...
    .catch(async (error) => {
      ...
      await updateStreamStatus("complete_market_history_v4", "error", error instanceof Error ? error.message : String(error));
    });
