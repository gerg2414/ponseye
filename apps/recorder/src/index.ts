...
async function main() {
  ...
  void getAccessToken()
    .then(async (auth) => {
      const repairFeed = "complete_market_history_v5";
    ...
    .catch(async (error) => {
      ...
      await updateStreamStatus("complete_market_history_v5", "error", error instanceof Error ? error.message : String(error));
    });
