  ...
  with selectDid as (
    select jsonb_build_object(
      ...
      'chartCandles', coalesce((
        with launch_state as (
          select l.graduated_at
        ...
        metric_quote_scale as (
          select m.price_usd / nullif(m.last_unit_price_raw, 0) as scale
          from public.launch_metrics m
          cross join launch_state ls
          where m.token_address = lower(p_token_address)
            and ls.graduated_at is null
            and m.price_usd > 0
            and m.last_unit_price_raw > 0
        ),
        direct_quote_scale as (
          ...
          cross join launch_state ls
          where ls.graduated_at is not null
            and rcp.raw_quote_price > 0
            and md.price_usd > 0
        ),
        migration_anchor as (
          select public.get_launch_migration_price_usd(l.token_address) as price_usd
          from public.launches l
          where l.token_address = lower(p_token_address)
        ...
        quote_scale as (
          select coalesce(
            ...
            dqs.scale,
            (
              select ma.price_usd / rcp.raw_quote_price
              from raw_curve_points rcp
              cross join migration_anchor ma
              where rcp.raw_quote_price > 0
                and ma.price_usd > 0
              order by rcp.block_time desc, rcp.sort_id desc
              limit 1
            )
          ) as scale
          from direct_quote_scale dqs
          left join metric_quote_scale mqs on true
        ),
        chart_points as (
          select
            rcp.block_time,
      ...
      'bondPriceUsd', coalesce((
        ...
        select public.get_launch_migration_price_usd(l.token_address)
        from public.launches l
        where l.token_address = lower(p_token_address)
          and l.graduated_at is not null
      )),
...
comment on function public.get_launch_detail(text, integer, integer) is
  'Returns complete curve and pool OHLC candles, using the corrected live quote scale for non-graduated tokens without stored market rows.';
