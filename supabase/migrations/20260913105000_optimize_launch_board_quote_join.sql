...
select
  ...
  case
    when r.graduation_threshold_raw > 0 then least(100::numeric, round(
      greatest(coalesce(m.net_quote_raw, 0), 0) * 100 / r.graduation_threshold_raw, 1))
    else null
  end as progress_pct,
...
comment on view public.launch_board is
  'Server-only launch board metrics, including the curve starting valuation before the first trade.';
