...
as $$
  select jsonb_build_object(
    ...
    'chartCandles', coalesce((
      ...
      migration_anchor as (
        select public.get_launch_migration_price_usd(lower(p_token_address)) as price_usd
      ),
      quote_scale as (
        select coalesce(
          dqs.scale,
    ...
    'bondPriceUsd', coalesce((
      ...
      limit 1
    ), public.get_launch_migration_price_usd(lower(p_token_address))),
    'poolStartedAt', (
...
comment on function public.get_launch_detail(text, integer, integer) is
  'Returns complete curve and pool OHLC candles, using the Pons migration endpoint to price raw curve history when direct priced curve rows are unavailable.';
